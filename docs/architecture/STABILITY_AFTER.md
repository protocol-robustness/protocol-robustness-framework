...
| Cancellation classification | base `:decision :classification` | recomputation in admission requires the declared evaluation to match a freshly planned one and to classify `:authorized`, else `:authority/forbidden` blocks (`cancellation/admission.clj:130,147`; produced by `cancellation/ordinary-planner.clj`) | `:authorized`, `:forbidden` only; `:forbidden-authorized` is a notebook scenario label, never canonical; no `:authorized-by-override` exists in code |
