...
| Serialization / fence / version | cfa lifecycle cutpoint + window classification; Surface B carries `:lifecycle-head-root` and snapshot fence | single fencing rule both surfaces obey (window gate + head-root match); no transition without a current fence |
