...
(defn snapshot-default-state
  "Snapshot process-global state before/after an isolated run.
   ...
   Returns {:gating {...} :advisory {...}}:
     :gating  — evidence/node/attestation registries + shared artifact dir path;
                these MUST be unchanged by isolated runs (hard gate).
     :advisory — system properties, user.dir, locale/timezone, uncaught
                 exception handler, the live thread set, and the artifact file
                 listing; changes are reported but do not fail the run (avoid
                 false positives from JVM/agent activity or test-generated
                 artifacts)."
  []
...
(defn- finalize
  ...
  (let [per-ns (vec (mapcat :per-ns group-results))
    ...
    ;;    shared artifact dir.  Advisory diffs (artifact-dir file listing,
    ;;    system props, locale, threads) are reported but never fail.
    (when leak?
