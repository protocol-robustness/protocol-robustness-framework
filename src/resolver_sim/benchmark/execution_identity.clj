...
(defn descriptor [scenario-source scenario repetition-index]
  (let [protocol (or (:protocol scenario) "sew-v1")]
    {:execution/version 1
     ...
     :input/content-hash (hash-ref/sha256-ref (input-source/sha256 scenario-source))
     :parameter-set-hash (hash-ref/sha256-ref (canonical/domain-hash
                                               "BENCHMARK_EXECUTION_PARAMETERS_V1"
                                               (or (:params scenario) {})))
     :protocol-config-hash (hash-ref/sha256-ref (canonical/domain-hash
...
(defn execution-id [descriptor]
  (hash-ref/sha256-ref (canonical/domain-hash
                        "BENCHMARK_EXECUTION_DESCRIPTOR_V1"
                        descriptor)))

(defn directory-name [ordinal descriptor]
  (let [hash (hash-ref/parse-sha256-ref (execution-id descriptor))]
