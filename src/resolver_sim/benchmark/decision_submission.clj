...
(defn submit-decision!
  "Validate and immutably retain one governed decision. This never counts a
   threshold, issues authority, or consumes a permit. `store` is an atom whose
   :decisions map is keyed by decision root."
  [store {:keys [decision authorisation-id request-root outcome-root
                 review-round review-governance signer-key-set]}]
  (cond
    (not (map? decision)) (reject :malformed-decision)
    (not (and (map? review-round) (map? review-governance) (map? signer-key-set)))
    ...
    (not= (:review-round/hash decision)
          (:review-round/hash review-round)) (reject :round-mismatch)
