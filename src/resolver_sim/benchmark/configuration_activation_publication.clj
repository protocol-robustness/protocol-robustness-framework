...
(defn activate-under-verified-transition-authorization!
  ...
  (let [current @(.state authority-store)
        ...
        successor-schema (:configuration/schema successor-configuration)
        v1-successor? (= genesis/chain-configuration-schema successor-schema)
        v2-successor? (= genesis/chain-configuration-v2-schema successor-schema)
        v3-successor? (= genesis/chain-configuration-v3-schema successor-schema)
        semantics-request? (or successor-semantics-policy successor-semantics)
    ...
    (cond
      ...
      (not (or v1-successor? v2-successor? v3-successor?))
      {:activated? false :reason :unsupported-successor-configuration-schema}
      (and (or v2-successor? v3-successor?)
           (not (and successor-semantics-policy successor-semantics)))
      {:activated? false :reason :successor-authority-semantics-incomplete}
      (and v1-successor? semantics-request?)
      {:activated? false :reason :authority-semantics-not-supported-by-successor}
      (and v3-successor? (not successor-allocation-entitlement-policy))
