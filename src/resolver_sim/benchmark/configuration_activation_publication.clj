    ...
    (let [current @(.state authority-store)
      ...
      (cond
        ...
        (let [h1 (:configuration/head derived)
              ...
              next (-> current
                       (assoc :head e1-root)
                       (assoc-in [:envelopes e1-root] e1)
                       (assoc-in [:by-state (:execution/state-root e1)] e1-root)
                       (assoc-in [:material (:execution/state-root e1)] successor-material)
                       (assoc-in [:configuration-head-states (:configuration-head/root e1)] h1)
                       (assoc-in [:activation-lineage e1-root] lineage))]
          (if (not= current @(.state authority-store))
            (recur)
            (if (compare-and-set! (.state authority-store) current next)
