...
(defn submission [root]
  (try
    (let [root (io/file root) completion (io/file root "completion.json") c (readj completion)
      ...
      (if (and (= (get c "run_package_index_sha256") (sha index))
               (= (get c "run_package_index_bytes") (.length index))
               (= (get p "policy_sha256") (str "sha256:" (canonical/domain-hash "PRF_VERDICT_POLICY_V1" (dissoc p "policy_sha256")))))
        {:valid? true
        ...
        {:valid? false :reason :invalid-package-seal}))
    (catch Exception _ {:valid? false :reason :unreadable-package})))
